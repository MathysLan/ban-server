...
// la data-URL envoyée par A ressort À L'OCTET PRÈS chez B (lobby, ordre de passage, classement, podium), l'emoji de B ressort chez A, et chaque avatar
...
(async () => {
  ...
  // ═══ 2. une vraie vidéo : découverte, deux tours passés par le MJ, résultats
  a.send({ action: 'start', videos: 1 });
  const pv = await b.wait(phase('preview'));
  t('en jeu (ordre de passage) : PP de A, emoji de B',
    same(find(pv.order, ja.id).avatar, IMG_A) && same(find(pv.order, jb.id).avatar, EMO_B));
  a.send({ action: 'next' });
  await b.wait(phase('turn'));
  a.send({ action: 'skip' });
  await b.wait(type('stopped'));
  a.send({ action: 'next' });
  await b.wait(phase('turn'));
  a.send({ action: 'skip' });
  await b.wait(type('stopped'));
  ...
  t('résultats (classement) : PP de A retransmise', same(find(res.ranking, ja.id).avatar, IMG_A));
  t('résultats (scores) : PP de A, emoji de B',
    same(find(res.scores, ja.id).avatar, IMG_A) && same(find(res.scores, jb.id).avatar, EMO_B));
  a.send({ action: 'next' });
